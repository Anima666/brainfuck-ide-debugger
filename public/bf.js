...
function get_input() {
  ...
  // console.log(  g_memory[g_mp]);
  // if (boolmodal == true){
  //   var data = $("#inputinmodal").val();
  //   console.log(data);
  // if (data.charAt(0) == '#') {
  //g_memory[g_mp] = parseInt(data.substr(1), 10);
}
...
function addLeadingZeros(n, length) {
  if (length == 8) {
    ...
    while (i < length - n.toString().length) {
      result = "0" + result;
      i++;
    }
...
function transformtodots(n) {
  if ((parseInt(n) >= 0) && (parseInt(n) <= 31))
    return ".";
  else {
...
function createleftpanel() {
  ...
  var lines = [];
  for (var x = 0; x < 16; x++) {
    var add = addLeadingZeros(label, 8);
    lines.push(add);
    label += 10;
    var html = lines.join(g_linebreaker);
    var p_node = document.getElementById('memview3');
    p_node.innerHTML = html;
  }
}

function changeselemmemory(idx, value) {
  g_memory[idx] = value;
...
function pad_num(a, b) {
  var c = new String(a);
  for (var i = c.length; i < b; i++)
    c = '0' + c;
  //return decimalToHexString(parseInt(c));
  return c;
}
...
function debug_toggle(f) {
  if (g_debugging == 1) {
    ...
    set_viewdata('memview2', ' '); //я колхожу
